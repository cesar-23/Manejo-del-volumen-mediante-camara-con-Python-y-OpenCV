...
#cap = cv2.VideoCapture('videoEntrada3.mp4')
bg = None
	...
	if bg is not None:
		...
		for cnt in cnts:
			...
			if defects is not None:
				...
				#Funciones que procesan el numero de dedos y ejecutan ordenes al ordenador

				
				if (fingers == 0) and Cambio:						#controlador de cambio
					Cambio = False

				if (fingers == 1) and not Cambio:					#Sube el volumen	
					pa.press('volumeup')
					Cambio = True
				
				if (fingers == 2) and not Cambio:					#Baja el volumen
					pa.press('volumedown')
					Cambio = True

				if (fingers == 3) and not Cambio:					#abrir un app, por ejemplo el calendario
					pa.press('win')
					pa.write('Calendario')
					pa.press('enter')
					Cambio = True

				if (fingers == 4) and not Cambio:					#Inicializa un programa e introduce una serie de datos
					pa.press('win')
					pa.write('Navegador Opera', interval=0.01)
					pa.press('enter')
					pa.press('alt'+'tab')
					pa.press('alt'+'tab')
					pa.moveTo(200,100, duration=2, tween=pa.easeInOutQuad)
					pa.click()
					pa.write('https://dutic.unsa.edu.pe', interval=0.01)
					pa.press('enter')
					pa.moveTo(1780,200, duration=2, tween=pa.easeInOutQuad)
					pa.click()
					pa.moveTo(1780,250, duration=2, tween=pa.easeInOutQuad)
					pa.click()
					pa.moveTo(200,525, duration=2, tween=pa.easeInOutQuad)
					pa.click()
					pa.moveTo(990,600, duration=2, tween=pa.easeInOutQuad)
					pa.click()
					Cambio = True

				if (fingers == 5) and not Cambio:					#cerrar un programa
					pa.moveTo(1890,45)
					pa.click()
					Cambio = True
